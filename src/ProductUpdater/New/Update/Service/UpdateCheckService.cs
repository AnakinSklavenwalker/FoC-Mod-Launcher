...
namespace ProductUpdater.New.Update.Service
{
    public class UpdateCheckService : IUpdateCheckService
    {
        ...
        private Task<IUpdateCatalog> CheckForUpdatesInternalAsync(UpdateRequest updateRequest, CancellationToken token)
            => Task.Run(() =>
            {
                ValidateRequest(updateRequest);
                var currentCatalog = _productService.GetInstalledProductCatalog();
                var availableCatalog = _productService.GetAvailableProductManifest(updateRequest.ManifestLocation);
        ...
        {
            if (updateRequest.ManifestLocation.Product is null)
            {
            ...
            var manifestUri = updateRequest.ManifestLocation.UpdateManifestPath;

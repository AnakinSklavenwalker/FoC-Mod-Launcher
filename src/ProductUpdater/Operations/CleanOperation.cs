...
namespace ProductUpdater.Operations
{
    internal class CleanOperation : IOperation
    {
        ...
        public CleanOperation(IServiceProvider serviceProvider)
        {
            ...
            _pipelineRunner = new AsyncPipelineRunner(ConcurrentClean);
        ...
        public bool Plan()
        {
            ...
            foreach (var data in files)
            {
                ...
                if (!File.Exists(file)) 
                    continue;
                var cleanTask = new CleanFileTask(data.Key, file, _serviceProvider.GetRequiredService<IFileSystem>(), _logger);
